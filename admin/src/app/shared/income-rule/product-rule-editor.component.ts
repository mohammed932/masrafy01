                      ...
                      <div class="slot" [class]="'is-' + slot.shape">
                        ...
                        @switch (slot.shape) {
                          ...
                          @case ('bands') {
                            ...
                            <!-- The three-state inherited/own/none row that stood here was
                                 the I-Score tier table's, and only ever its: it was the one
                                 member of the server's inherited-when-blank slot set. The
                                 tiers are not a step any more (v30.3.0) - they live on the
                                 Requirements card of the bank wizard and on the product's own
                                 screen, where every program type can reach them. No slot in a
                                 rule reads the product's figures when blank now: a blank way
                                 or condition means this bank does not sell that, which is a
                                 stated decision the product must not override. -->
...
export class ProductRuleEditorComponent {
  ...
  private stepRow(step: RuleStep): EditorRow {
    ...
    return {
      ...
      // The INHERITED exception that stood here belonged to the I-Score slot alone, which is
      // no longer a step (v30.3.0). No slot in a rule reads the product's figures when blank
      // any more, so a blank optional row is "not used by this bank" again, with nothing to
      // misread.
