...
/// [MasrafyBottomSheetBase] instead — it composes this shell from a small set
/// of hooks.
///
...
/// footer.
class MasrafyBottomSheetShell extends StatelessWidget {
  const MasrafyBottomSheetShell({
    super.key,
...
/// the `MasrafyAppBar` pattern: one abstract base, many concrete subclasses.
///
/// For sheets with internal state (e.g. a calendar with month navigation),
/// use [MasrafyBottomSheetShell] directly from inside a `State.build`.
///
/// Present via [MasrafyBottomSheetBase.show]:
/// ```dart
/// final picked = await MasrafyBottomSheet.show<int>(
///   context: context,
///   sheet: const _MySheet(),
/// );
/// ```
abstract class MasrafyBottomSheetBase extends StatelessWidget {
  const MasrafyBottomSheetBase({super.key});
  ...
  Color? dragHandleColor(MasrafyColorTheme colors) => colors.border.secondary;
