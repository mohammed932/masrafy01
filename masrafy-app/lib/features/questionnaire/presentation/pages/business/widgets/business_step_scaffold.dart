...
/// Collapse-on-scroll host shared by the four business-loan wizard step bodies
/// (Figma `4024:2741`+). Each step is its OWN [CustomScrollView] with a pinned
...
/// semantics ([BusinessQuestionnaireState.totalSteps]) and back behaviour. The
/// gradient/glass-back/collapse styling stays in the shared
/// [MasrafyGradientHeader] + [MasrafySliverGradientHeaderDelegate].
class BusinessStepScaffold extends StatelessWidget {
  const BusinessStepScaffold({
    super.key,
    required this.title,
      ...
      slivers: [
        SliverPersistentHeader(
          ...
          delegate: MasrafySliverGradientHeaderDelegate(
            title: title,
            subtitle: l.q_business_subtitle,
            onBack: onBack,
            bottom: MasrafySegmentedProgress(
              total: BusinessQuestionnaireState.totalSteps,
              current: stepIndex,
            ),
