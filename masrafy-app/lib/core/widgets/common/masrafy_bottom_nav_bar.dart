...
enum MasrafyNavTab {
  ...
  static MasrafyNavTab? fromIndex(int index) =>
      index >= 0 && index < MasrafyNavTab.values.length
          ? MasrafyNavTab.values[index]
          : null;
}
...
class MasrafyBottomNavBar extends StatelessWidget {
  const MasrafyBottomNavBar({
    super.key,
  ...
  static const _tabs = [
    _NavTab(svgAsset: MasrafyAssets.kNavHome, label: 'Dashboard'),
    _NavTab(svgAsset: MasrafyAssets.kNavStudyPlanner, label: 'Study Planner'),
    _NavTab(svgAsset: MasrafyAssets.kNavTests, label: 'Test'),
    _NavTab(svgAsset: MasrafyAssets.kNavReports, label: 'Reports'),
    _NavTab(svgAsset: MasrafyAssets.kNavEshop, label: 'E-Shop'),
  ];
...
/// Vertical iPad-side navigation rail. Mirrors [MasrafyBottomNavBar]'s pill
/// style — same icons, same active treatment — but stacked vertically with
/// the label always visible on the active pill.
class MasrafyNavigationRail extends StatelessWidget {
  const MasrafyNavigationRail({
    super.key,
    required this.currentIndex,
    required this.onTabSelected,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            for (var i = 0; i < MasrafyBottomNavBar._tabs.length; i++) ...[
              _RailItem(
                key: i == 2 ? testItemKey : null,
                tab: MasrafyBottomNavBar._tabs[i],
                isActive: i == currentIndex,
                onTap: () {
                  HapticFeedback.selectionClick();
                  onTabSelected(i);
              ...
              if (i < MasrafyBottomNavBar._tabs.length - 1) SizedBox(height: 12.h),
            ],
          ],
        ),
      ),
...
/// [MasrafyNavigationRail] on tablets, or at the bottom as a
/// [MasrafyBottomNavBar] on phones. Use for every root tab screen.
class MasrafyNavScaffold extends StatelessWidget {
  const MasrafyNavScaffold({
    super.key,
    required this.body,
    required this.currentIndex,
    required this.onTabSelected,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      endDrawer: endDrawer,
      body: body,
      bottomNavigationBar: MasrafyBottomNavBar(
