...
**(v29.0.0) A second, narrower axis: the program the applicant picked.** `GET /v1/questionnaire?category=&programNameKey=` serves the category's asked set INTERSECTED with the questions a program behind that name can be quoted from, plus a fixed CORE every quote needs — so one program asks three questions and another fifteen, on either income basis. It is still ONE pool and ONE snapshot: the narrowing is a per-request projection, read LIVE (its inputs are bank-program rows and the ask table, not questionnaire content), derived by ONE shared pure function (`questionnaire/validation/question-scope.ts`) from ONE program set — the set `matchesRequestedScope` selects for the request's own triple — so what an applicant is ASKED and what they are QUOTED are two readings of the same choice. What a program reads comes from `effectiveIncomeRule` plus its cap table, never the raw stored column. The set is gate-closed over `enabledWhen`, the CORE is taken from the existing money/obligation/reserved constants and never hand-typed, and a served question the picked program reads is REQUIRED. `programNameKey` narrows what is asked and required; it never narrows what may be ANSWERED. An unknown or unofferable name fails loudly through the same `assertOfferedUnder` apply and preview use. **(v29.1.0) The operator adjusts the axis per (name, loan type) with two lists, both read LIVE with the rest of it:** questions the name SKIPS (`program_name_question_exclusion`, bounded by `questionLockReason` — the quote's own inputs and what a program under the name reads cannot be skipped) and questions it ADDS (`program_name_question_addition`). A question the loan type does not ask of every name is added through an OPT-IN row (`question_loan_category.optIn`), frozen at publish as `optInCategories` — disjoint from `categories` — so the axis stays a NARROWING of the category's frozen set: an opt-in row is never core, is served only where the name added it or a program under it reads it, and never with no name. Added means asked, never required because of it; answers stay accepted category-wide. See A33.
...
**(v29.0.0 — the program-name axis.)** A SECOND, narrower axis sits inside the category: the catalog `program_name` the applicant picked. Serving only the questions a program behind that name can be quoted from, plus a fixed core, is legal — one program may need three questions and the next fifteen, and it applies to BOTH income bases, because a payslip program reads facts through its cap table exactly as a no-payslip program reads them through its calculation. It is a per-REQUEST projection of the category's asked set, never a second pool and never a write to `question_loan_category`. Unlike the category axis it is **read LIVE and MUST NOT be frozen into the snapshot** — a `programNames` key beside each question's `categories`, one snapshot per name, or any stored list of "the questions this product needs" = review block. Its inputs are `surrogate_product_ask`, `bank_program.incomeAssumption`, `bank_program.loanLimits` and `platform_enumeration.incomeRule`, none of which is questionnaire content, and `factsReadBy` already forbids the shape: "DERIVED, never stored … a second list saying which questions the product needs could only ever drift out of step with it." A frozen copy would also go stale in the direction that RAISES a ceiling, because editing a cap table publishes no questionnaire version: the stale copy keeps hiding a question the cap started reading, `maxLoanByFact` misses, and `onNoMatch: 'useProgramMax'` quotes the program's own maximum instead of the bank's row, silently. Equally a review block: deriving the narrowed set anywhere but the one shared `questionnaire/validation/question-scope.ts`; deriving what a program reads from the raw `incomeAssumption` column instead of `effectiveIncomeRule` (a program on catalog amounts stores no steps of its own and would read as needing nothing); a narrowed set that is not GATE-CLOSED over `enabledWhen`, transitively and to a fixpoint (a dangling gate makes its target unconditionally VISIBLE, so dropping a source while a required question behind it survives makes that question mandatory and unanswerable); a hand-typed core list instead of the existing `MONEY_FIELD_BINDINGS` / `DEBT_TYPES_QUESTION_CODE` / `OBLIGATION_ITEM_QUESTION_BY_DEBT_TYPE` / `RESERVED_FACT_KEYS` constants; a name-filtered read that falls back to the whole set on an unknown name instead of raising `PROGRAM_NAME_KEY_UNKNOWN`; and **rejecting an answer to a question the CATEGORY asks merely because the picked name did not need it** — the name narrows what is ASKED and REQUIRED, never what may be ANSWERED, because a backend deploy is not atomic with an app release and every installed build posts the whole category set. **(v29.1.0 — the operator's two lists.)** The axis may also be adjusted per (name, loan type) by an operator: a SKIP list (`program_name_question_exclusion`) and an ADD list (`program_name_question_addition`). These are the operator's stated policy, not "a stored list of the questions this product needs" — that list is still derived and never stored, and both lists are bounded by it: a skip of a question the quote or a program under the name reads is ignored (`questionLockReason`), and an addition only ever keeps more. Both are read LIVE and MUST NOT be frozen. A question added for a name that the category does not ask of every name sits in the category as an OPT-IN row (`question_loan_category.optIn`), which IS category content and IS frozen, as `optInCategories`, disjoint from `categories` — so a reader that has never heard of it treats the row as unassigned (not served, not required), and the name axis remains a projection of the category's frozen set rather than a widening of it. Review blocks: an opt-in row treated as core (clause 1 alone would ask it of every name), an opt-in row served with no `programNameKey`, an addition made REQUIRED by being added (a question keeps its own requiredness; `extraRequired` comes from what a program reads), acceptance narrowed to the name's list, a writer of `question_loan_category` that drops or resets `optIn` (the replace writers carry it; a global add turns the row ordinary; a re-seed never deletes an opt-in row), and an engine input (`questionLockReason` = `engine`) added for one name — whether the quote's inputs are asked is the loan type's call.

**Preview and apply deriving the same engine input differently = review block.** Question visibility comes from the one shared `validation/question-visibility.ts`, the asked-question scope from the one shared `validation/question-scope.ts`, the surrogate facts from the one shared `surrogate-facts-from-answers.ts`, and the order from the one shared `rankOffers` — the same answers must not produce two outcomes either side of apply.
...
| 25.1.0–28.0.0 | 2026-09-05 – 2026-09-09 | — | **No principle text was amended.** Four releases of product work — the wizard's income step, the no-payslip product board, the I-Score tier tables, the Suez Canal auto mechanism, and the sheet conditions — recorded in `CLAUDE.md`'s Recent Changes. Listed so the jump from 25.0.0 to 29.0.0 does not read as contiguous. |
| 29.1.0 | 2026-09-26 | MINOR | **Principle V — the operator adjusts the program-name axis per (name, loan type).** Two live lists: a name SKIPS a question its loan type asks (`program_name_question_exclusion`, bounded by `questionLockReason`: the money/debt/employment inputs, platform facts and anything a program under the name reads cannot be skipped) and ADDS one it does not (`program_name_question_addition`). An added question the category does not ask of every name sits in it as an OPT-IN row (`question_loan_category.optIn`), frozen as `optInCategories` disjoint from `categories`, so the axis stays a narrowing of the frozen set: opt-in is never core, never served without a name, served where the name added it or a program reads it. Added never means required; acceptance stays category-wide; engine inputs stay loan-type-wide. `narrowAskedQuestions` now returns an authoritative `keep` with narrowing off too (the category minus opt-in rows nobody added, gate-closed), and every caller honours it. A33 amended. |
| 29.0.0 | 2026-09-10 | MAJOR | **Principle V — every program asks only its own questions.** A second, narrower axis sits inside the loan category: the catalog `program_name` the applicant picked. `GET /v1/questionnaire?category=&programNameKey=` serves that category's asked set intersected with the questions a program behind the name can be quoted from, plus a core every quote needs — 57 personal questions become 24–40 depending on the name, and 23 required become 12–23. It applies to BOTH income bases, because a payslip program reads facts through `loanLimits.maxLoanByFact` exactly as a no-payslip program reads them through its calculation; keying on the surrogate product instead would have hidden four cap facts from `private_sector`'s programs and quoted them above their own bank's table through `onNoMatch: 'useProgramMax'`, silently. Still ONE pool and ONE snapshot: the narrowing is a per-request projection, read LIVE and deliberately NOT frozen (its inputs are bank-program rows and `surrogate_product_ask`, not questionnaire content, and a frozen copy goes stale in the ceiling-raising direction), derived by one shared pure `questionnaire/validation/question-scope.ts` that serve, apply and preview all call. CORE is derived from the existing `MONEY_FIELD_BINDINGS` / `DEBT_TYPES_QUESTION_CODE` / `OBLIGATION_ITEM_QUESTION_BY_DEBT_TYPE` / `RESERVED_FACT_KEYS` constants plus "no product asks this fact", never a hand list — the last clause is what stops one tick on one product's board un-asking a question for every other name. Gate handling is a fixpoint in both directions with the post-condition asserted, because `isQuestionVisible` shows a question whose gate source is missing and a required one would then be mandatory for everyone. A served question the program reads is REQUIRED; `programNameKey` never narrows what may be ANSWERED, so every installed app build keeps applying. No new error code (`assertOfferedUnder` refuses an unknown name), no migration, no snapshot field. New `npm run check:question-scope` asserts no fact a live program reads is hidden and prints the served/required table; `planDetach` now refuses an untick while any bank program's rule or cap still reads the fact, which is the hole `do_you_own_more_than_one_unit` went through. A33 and A26 amended; Principle V's parity list gains the new function. |

---

**Version**: 29.1.0 | **Ratified**: 2026-05-12 | **Last Amended**: 2026-09-26
