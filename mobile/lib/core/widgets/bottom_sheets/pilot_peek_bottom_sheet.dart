...
///
/// Provide a fresh [PilotPeekSheetController] to programmatically toggle the
/// snap (used by the editor cubit when an answer is picked).
class PilotPeekBottomSheet extends StatefulWidget {
  const PilotPeekBottomSheet({
    super.key,
  ...
  final PilotPeekSheetController? controller;
  final double minChildSize;
  final double maxChildSize;

  @override
  State<PilotPeekBottomSheet> createState() => _PilotPeekBottomSheetState();
}

class _PilotPeekBottomSheetState extends State<PilotPeekBottomSheet> {
...
/// Programmatic snap toggle. Pass to [PilotPeekBottomSheet.controller] and
/// call [collapse] / [expand] from outside the sheet (e.g. after an answer
/// pick).
class PilotPeekSheetController {
  PilotPeekSheetController();

  DraggableScrollableController? _drag;
  // ignore: unused_field
  _PilotPeekBottomSheetState? _state;

  void _attach(DraggableScrollableController c, _PilotPeekBottomSheetState s) {
    _drag = c;
    _state = s;
  }
