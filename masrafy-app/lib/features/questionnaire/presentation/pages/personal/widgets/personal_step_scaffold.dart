...
/// Collapse-on-scroll host shared by the four personal-loan wizard step bodies
/// (Figma `4024:2442`+). Each step is its OWN [CustomScrollView] with a pinned
...
/// semantics ([PersonalQuestionnaireState.totalSteps]) and back behaviour. The
/// gradient/glass-back/collapse styling stays in the shared
/// [MasrafyGradientHeader] + [MasrafySliverGradientHeaderDelegate].
class PersonalStepScaffold extends StatelessWidget {
  const PersonalStepScaffold({
    super.key,
    required this.title,
      ...
      slivers: [
        SliverPersistentHeader(
          ...
          delegate: MasrafySliverGradientHeaderDelegate(
            title: title,
            subtitle: l.q_personal_subtitle,
            onBack: onBack,
            bottom: MasrafySegmentedProgress(
              total: PersonalQuestionnaireState.totalSteps,
              current: stepIndex,
            ),
