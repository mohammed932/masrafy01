...
///
/// Each row label renders as `"$code - $label"` when [PilotMultiSelectOption.code]
/// is non-empty, otherwise just [PilotMultiSelectOption.label]. Locked
/// options render at 25% opacity and ignore taps (mirrors Angular
/// `plan-form.component.ts#isExistingSubject`).
Future<List<T>?> showPilotMultiSelectSheet<T>({
  required BuildContext context,
  required String title,
  required List<PilotMultiSelectOption<T>> options,
...
class PilotMultiSelectOption<T> {
  const PilotMultiSelectOption({
    required this.value,
    required this.label,
    this.code = '',
    this.isLocked = false,
...
class _MultiSelectSheet<T> extends StatefulWidget {
  ...
  final String title;
  final List<PilotMultiSelectOption<T>> options;
...
class _MultiSelectSheetState<T> extends State<_MultiSelectSheet<T>> {
  ...
  List<PilotMultiSelectOption<T>> get _filtered {
  ...
  void _toggle(PilotMultiSelectOption<T> option) {
...
class _Content<T> extends StatelessWidget {
  ...
  final String searchHint;
  final List<PilotMultiSelectOption<T>> rows;
  final Set<T> selected;
  final void Function(PilotMultiSelectOption<T>) onToggle;
  final String emptyMessage;
...
class _OptionRow<T> extends StatelessWidget {
  ...
  final PilotMultiSelectOption<T> option;
