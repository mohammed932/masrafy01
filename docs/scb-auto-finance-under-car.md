...
- **Catalog names** — `PROGRAM_NAMES` in [sheet-figures.ts:449-515](backend/src/bank-programs/demo-figures/sheet-figures.ts#L449-L515), all `categories: [car]` (the command writes `incomeBases ['no_payslip']` per category): `auto_down_payment_income` "Auto Loan — Down Payment as Income" / "قرض سيارة — الدفعة المقدمة كدخل" → `down_payment_income`; `green_power_loan` "Green Power Loan" / "قرض الطاقة الخضراء" → `savings_income`; `micro_mobility` "Micro Mobility" / "التنقل الخفيف" → `savings_income` (two names so the applicant picks the product they are buying; both quote off one calculation). Fix the "personal only" comment at :440-447.
