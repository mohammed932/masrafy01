...
///
/// Owns the single `PilotShimmer` wrapper — callers MUST NOT wrap in
/// another `PilotShimmer` (Principle X: one shimmer per skeleton).
class PilotShimmerLeaderboardRow extends StatelessWidget {
  const PilotShimmerLeaderboardRow({
    super.key,
    required this.body,
    ...
    final row = Row(
      children: [
        SizedBox(
          ...
          child: PilotShimmerLine(width: rankLineWidth, height: 14),
        ),
        Gap(gap.w),
        PilotShimmerCircle(diameter: avatarDiameter),
        Gap(gap.w),
        Expanded(child: body),
        trailing,
      ],
