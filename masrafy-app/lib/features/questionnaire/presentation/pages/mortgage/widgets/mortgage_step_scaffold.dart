...
/// Collapse-on-scroll host shared by the four mortgage wizard step bodies
/// (Figma `4024:2197`+). Each step is its OWN [CustomScrollView] with a pinned
...
/// semantics ([MortgageQuestionnaireState.totalSteps]) and back behaviour. The
/// gradient/glass-back/collapse styling stays in the shared
/// [MasrafyGradientHeader] + [MasrafySliverGradientHeaderDelegate].
class MortgageStepScaffold extends StatelessWidget {
  const MortgageStepScaffold({
    super.key,
    required this.title,
      ...
      slivers: [
        SliverPersistentHeader(
          ...
          delegate: MasrafySliverGradientHeaderDelegate(
            title: title,
            subtitle: l.q_mortgage_subtitle,
            onBack: onBack,
            bottom: MasrafySegmentedProgress(
              total: MortgageQuestionnaireState.totalSteps,
              current: stepIndex,
            ),
