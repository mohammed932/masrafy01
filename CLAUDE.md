...
Constitution: [.specify/memory/constitution.md](.specify/memory/constitution.md) v14.0.0
...
- 2026-08-04 (v12.0.0): MAJOR — **Principle V: questions are ASSIGNED to loan categories.** Pool stays ONE global list + ONE versioned snapshot; new `question_loan_category` join table (many-to-many) decides which categories ask each question. Migration `question_loan_category_assignment` creates it and backfills all four categories for every existing question (no behaviour change until an admin narrows one). Backend: `activeSnapshot(category?)`, `GET /v1/questionnaire?category=` (unknown → `VALIDATION_FAILED`), `resolveAnswers(answers, category?)` scopes required-question enforcement (apply passes `dto.category`), `publish()` freezes `categories` per question, `draftTree()` returns them, `CreateQuestionDto.categories?` defaults to all four, new `PUT /admin/questionnaire/questions/:id/categories` + `POST /admin/questionnaire/questions/categories` (bulk: one transaction, one publish). Admin: `/questionnaire` is now a two-tab route shell — `questions` (existing pool builder, untouched) and `categories` (new `question-categories.page.ts`: question × category tick matrix, column All/None, parked + dangling-branch warnings). Mobile: `getActive(category)` threaded datasource → repo → usecase → cubit; `QuestionnaireView` passes its category. New admin i18n ids `qtabs.*` / `qcat.*` (ar translated). No new error codes. Constitution v12.0.0; A33 reworded (still blocks a `category` column + per-category questionnaires).
- 2026-08-04 (v11.0.0): MAJOR — **Principle XIII: SOCIAL sign-in is Google-only; Apple removed end to end.** Backend: `social/apple-verify.service.ts` deleted, `POST /v1/auth/social/apple` + `POST /v1/auth/apple/login` deleted, `SocialAppleSignInDto` deleted, `verifyProviderToken()` collapses to the Google verifier (`userInfo` param dropped), `APPLE_BUNDLE_ID` gone from `env.schema.ts` + both `.env.*.example`. Prisma `SocialProvider` → `{ GOOGLE }` via migration `google_only_social_provider` (enum recreated; APPLE links/sessions deleted; only accounts orphaned BY that delete are deactivated). Mobile: `sign_in_with_apple` package + pod removed, Apple datasource/repo/usecase methods deleted, `SocialProvider` → `{ google }`, Apple buttons removed from login + onboarding (the onboarding one silently ran the Google flow), ARB keys `auth_landing_action_apple`/`onboarding_apple`/`login_apple` deleted + the three "Google or Apple" strings reworded (ar + en, regenerated). Admin: same three strings in `error-codes.{ar-EG,en-US}.json`. No error code added/removed. **Same PR (A31 extended):** applicant `age` removed from every customer request body — `ApplyRequestDto.age` and `CalculatorQuoteDto.age` deleted, `MatchingPreviewService.preview()` takes a required `age`, all three derive it from the caller's `birthday` via the new `CustomerProfileCompletenessService.getApplicantAge()`; calculator gains `CustomerProfileCompleteGuard`; `ASSUMED_AGE = 18` deleted from calculator + preview (survives as `SIMULATOR_DEFAULT_AGE` for the admin path, with `age` on the new `SimulateMatchesDto`); mobile `ApplyRequest.age`/`withAge()` deleted and `MatchingResultsCubit` drops its `/auth/me` round-trip + `AuthUseCase` dep.
