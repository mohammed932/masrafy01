...
export class BanksApiService {
  ...
  async list(
    opts: { page?: number; pageSize?: number; search?: string; active?: boolean } = {},
  ): Promise<PaginatedEnvelope<BankWithProgramCount>> {
    ...
    return firstValueFrom(
      this.http.get<PaginatedEnvelope<BankWithProgramCount>>(this.base, { params }),
    );
  }

  async getById(id: string): Promise<SuccessEnvelope<BankWithProgramCount>> {
    return firstValueFrom(
      this.http.get<SuccessEnvelope<BankWithProgramCount>>(
  ...
  async listPrograms(id: string): Promise<SuccessEnvelope<BankProgramSummary[]>> {
    return firstValueFrom(
      this.http.get<SuccessEnvelope<BankProgramSummary[]>>(
        `${this.base}/${encodeURIComponent(id)}/programs`,
      ),
    );
  }
  ...
  async update(id: string, payload: BankUpdatePayload): Promise<SuccessEnvelope<Bank>> {
    return firstValueFrom(
      this.http.patch<SuccessEnvelope<Bank>>(`${this.base}/${encodeURIComponent(id)}`, payload),
    );
  }

  async toggle(id: string, payload: BankToggle): Promise<SuccessEnvelope<Bank>> {
    return firstValueFrom(
      this.http.patch<SuccessEnvelope<Bank>>(
        `${this.base}/${encodeURIComponent(id)}/toggle`,
        payload,
      ),
    );
  }
  ...
  async remove(id: string): Promise<SuccessEnvelope<{ id: string }>> {
    return firstValueFrom(
      this.http.delete<SuccessEnvelope<{ id: string }>>(`${this.base}/${encodeURIComponent(id)}`),
    );
  }
  ...
  async requestLogoUpload(
    id: string,
    contentType: string,
  ): Promise<SuccessEnvelope<BankPresignedLogo>> {
    return firstValueFrom(
      this.http.post<SuccessEnvelope<BankPresignedLogo>>(
        `${this.base}/${encodeURIComponent(id)}/logo/request-upload`,
        { contentType },
      ),
    );
  }
  ...
  async confirmLogoUpload(id: string, key: string): Promise<SuccessEnvelope<Bank>> {
    return firstValueFrom(
      this.http.post<SuccessEnvelope<Bank>>(
        `${this.base}/${encodeURIComponent(id)}/logo/confirm-upload`,
        { key },
      ),
    );
  }
