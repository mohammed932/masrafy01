...
/// [MasrafyDialogBase] instead — it composes this shell from a small set
/// of hooks.
///
...
class MasrafyDialogShell extends StatelessWidget {
  const MasrafyDialogShell({
    super.key,
...
/// the `MasrafyBottomSheetBase` pattern: one abstract base, many concrete
/// subclasses.
///
/// For dialogs with internal state (e.g. a multi-step picker), use
/// [MasrafyDialogShell] directly from inside a `State.build`.
///
/// Present via [MasrafyDialogBase.show]:
/// ```dart
/// final confirmed = await MasrafyDialogBase.show<bool>(
///   context: context,
///   dialog: const _MyDialog(),
/// );
/// ```
abstract class MasrafyDialogBase extends StatelessWidget {
  const MasrafyDialogBase({super.key});
  ...
  ///
  /// Re-installs the active [MasrafyColorTheme] inside the dialog route via
  /// [MasrafyColorThemeProvider] so the dialog's `MasrafyColorTheme.of(context)`
  ...
  static Future<T?> show<T>({
    ...
    return showDialog<T>(
      ...
      routeSettings: routeSettings,
      builder: (_) => MasrafyColorThemeProvider(theme: theme, child: dialog),
    );
  }
}

/// Canonical Cancel/Confirm action row for [MasrafyDialogBase] subclasses.
///
/// Used inside [buildFooter]:
/// ```dart
/// @override
/// Widget? buildFooter(BuildContext context) => MasrafyDialogActions(
///   cancelLabel: 'Cancel',
///   confirmLabel: 'Delete',
///   isDestructive: true,
...
/// );
/// ```
class MasrafyDialogActions extends StatelessWidget {
  const MasrafyDialogActions({
    super.key,
