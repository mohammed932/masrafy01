...
Constitution: [.specify/memory/constitution.md](.specify/memory/constitution.md) v6.0.0
...
- **V — Matching Engine Is IP** (v6.0.0): pure module. Formula + tiers in code (PR-reviewed). Approval `probability = Σ(points[questionCode][optionCode])/maxAchievablePoints` (clamped 0..1; `maxAchievablePoints` = per question the program's highest option points, summed). Questions + answer options are **pure content** (label + order — NO scoring/eligibility fields). Per bank program, points are assigned to answer **options** (nested `questionCode → optionCode → points` in `ScoringWeightSet.weights`, arbitrary scale, **no sum constraint**) — admin-editable DATA. **Direct admin save** (no maker-checker; editor ID audited; atomic archive+activate). No ACTIVE set → score 0. **Eligibility gating dropped for MVP** (no salary/age/DBR/loan-amount/max-loan): every active program returned ranked by probability, in BOTH preview and apply (apply runs the engine with `skipEligibility`). Questionnaire published as immutable versioned snapshots (content only — points never in the snapshot).
...
- **A33** Mutating ACTIVE weight set in place / hardcoded per-answer points / hand-typed questionnaire codes / adding scoring or eligibility fields back onto `Question`/`QuestionOption` / reintroducing eligibility gates into preview or apply / probability by any formula other than `Σ(picked points)/maxAchievablePoints` (Principle V, v6.0.0)
