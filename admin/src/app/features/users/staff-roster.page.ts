...
/**
 * Staff cohort of the People directory (`/people/staff`, super_admin only).
 * The header, primary action, and search box belong to `PeopleDirectoryPage` —
 * this roster owns the staff-specific stats, columns, and row actions, and
 * reads the shared query off `PeopleDirectoryStore`.
 */
@Component({
  ...
  template: `
    <div class="roster">
      ...
      <div class="table-panel">
        ...
        @if (!loading() && total() === 0) {
          @if (store.searching()) {
            <app-empty-state icon="group" [title]="noMatchTitle" [subtitle]="noMatchSubtitle" />
          } @else {
...
export class StaffRosterPage {
  ...
  protected readonly noMatchTitle = $localize`:@@users.empty.search.title:No staff match this search`;
  protected readonly noMatchSubtitle = $localize`:@@users.empty.search.subtitle:Staff are matched on name and email. Clear the search to see everyone.`;
  ...
  constructor() {
    ...
    effect(
      () => {
        this.store.query();
        // `reload()` reads page/pageSize — untracked so paging is not a trigger.
        untracked(() => {
          this.page.set(1);
          this.pageIndex = 1;
          void this.reload();
