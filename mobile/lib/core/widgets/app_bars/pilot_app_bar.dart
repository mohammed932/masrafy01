...
/// Icon-button action for the trailing area of a [PilotAppBar].
class PilotAppBarAction {
  const PilotAppBarAction({
  ...
  /// When true the button is filled with [PilotColorTheme.primary.main] and
  /// uses a white icon tint instead of the default bordered pill style.
  final bool isPrimary;
  final bool show;
}

/// Text-label action for the trailing area of a [PilotAppBar].
class PilotAppBarTextAction {
  const PilotAppBarTextAction({
    required this.label,
    required this.onTap,
    this.show = true,
  });
...
/// [autoBlur]).
abstract class PilotAppBar extends StatelessWidget implements PreferredSizeWidget {
  PilotAppBar({super.key, this.autoBlur = true});
  ...
  List<PilotAppBarAction> get trailingActions => const [];

  PilotAppBarTextAction? get trailingTextAction => null;
  ...
  /// that cannot be expressed as a simple [PilotAppBarAction].
  ...
  Widget _buildActionButton(PilotAppBarAction action, BuildContext context) {
    final colors = PilotColorTheme.of(context);
    return GestureDetector(
      onTap: action.onTap,
      child: Container(
